...
const CONFIG = {
  LINKS: {
    ...
    chart:
      "https://gmgn.ai/sol/token/BxBuSTek_3fzFmWRedsjuybCgic1rkkZAdKhzeHVvxs7sGBgrBAGS",
  ...
  initBiometric() {
    ...
    this.tg.BiometricManager.init((success) => {
      if (success && this.tg.BiometricManager.isBiometricAvailable) {
        console.log(
          "Biometric available:",
          this.tg.BiometricManager.biometricType,
        );
      }
    });
  }
...
class UIManager {
  ...
  async init() {
    ...
    try {
      ...
      if (stored && CONFIG.PALETTES.some((p) => p.id === stored))
        savedPalette = stored;
    } catch (e) {
      /* ignore storage errors */
    }
    ...
    if (telegram.user) {
      const name =
        telegram.user.first_name || telegram.user.username || "Citizen";
  ...
  bindEvents() {
    ...
    this.elements.loreToggle.addEventListener("click", () => {
      ...
      if (expanded) {
        this.elements.loreExtra.scrollIntoView({
          behavior: "smooth",
          block: "nearest",
        });
      }
    ...
    // Language switch
    this.elements.langBtnEn.addEventListener("click", () =>
      this.setLanguage("en"),
    );
    this.elements.langBtnTr.addEventListener("click", () =>
      this.setLanguage("tr"),
    );
    ...
    document
      ...
      ?.addEventListener("click", legalNotice);
    document
  ...
  shareApp() {
    ...
    if (navigator.share) {
      ...
      telegram.openLink(
        `https://t.me/share/url?url=${encodeURIComponent("https://t.me/wombit_io")}&text=${encodeURIComponent(text)}`,
      );
    }
  }
  ...
  setPalette(paletteId, announce) {
    const palette =
      CONFIG.PALETTES.find((p) => p.id === paletteId) || CONFIG.PALETTES[0];
    ...
    if (announce && this.elements.toast) {
      ...
      this.elements.toast
        ...
        ?.replaceChildren(document.createTextNode(`Theme: ${palette.id} ✅`));
      // Reuse the toast element generically for this announcement
      const originalHTML = this.elements.toast.innerHTML;
      this.elements.toast.innerHTML = isTr
        ? `Tema değişti: ${palette.id} ✅`
        : `Theme switched: ${palette.id} ✅`;
      this.showToast();
      setTimeout(() => {
        this.elements.toast.innerHTML = originalHTML;
      }, 2200);
    }
  }
}
